#!/usr/bin/env python3
"""
DSGRN Boolean Network Analysis
    ...
    def parse_dsgrn(self, spec: str) -> DSGRNNetwork:
        ...
        for line in spec.strip().split('\n'):
            ...
            for match in re.finditer(r'(~?)(\w+)', regulation):
                ...
                if source[0].isupper():  # Assume uppercase = node name
...
def main():
    """Run the DSGRN Boolean network analysis demo."""
    ...
    # Parse network
    parser = DSGRNParser()
    network = parser.parse_dsgrn(dsgrn_spec)
