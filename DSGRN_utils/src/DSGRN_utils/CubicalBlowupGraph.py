...
class CubicalBlowupGraph:
    ...
    The algorithm follows the definitions of `Rook_Field_Paper_v2`.  Passing
    `legacy=True` restores the pre-2026-07-28 behaviour of this file, which is
    needed to reproduce figures published before the definitions were aligned;
    the differences are documented at each use of `self.legacy` below and
    measured in `code/rookfields/reports/FINDINGS.md`.
    """
    ...
    def active_regulation_map(self, cc_cell):
        """Memoised wrapper around :meth:`compute_active_regulation_map`."""
    ...
    def semi_opaque_cell(self, cc_cell):
        """Memoised wrapper around :meth:`compute_semi_opaque_cell`."""
