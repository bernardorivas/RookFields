...
def ramp_system_wall_labelling(gamma, theta, h, f_ramp, global_bound=None):
    """Compute wall labelling from a ramp system.

    Definition `defn:ramp-wall-labeling` (RampSystemsv4.tex) uses the sentinel
    thresholds theta_{m_0} = 0 and theta_{m_{K(n)+1}} = GB_n, the global bound.
    Pass `global_bound` as a list of GB_n to use it.

    Without it the right-most wall falls back to theta + 10 h, which depends on
    h.  That contradicts `prop:wall-labeling-const` (the labelling is constant
    in h on H_0) and loses strong dissipativity once h is small - which is
    precisely the regime H_1 forces.  The fallback is kept only to reproduce
    figures produced before this was noticed.
    """
    # Space dimension
    dim = len(gamma)
    ...
    def cell_wall_label(top_cell):
        ...
        # Get the theta values at the right walls of cell (the sentinel theta_{m_{K+1}} = GB_n
        # at the right most wall, or the legacy theta + 10 * h if no global bound was given)
        theta_right = [theta_s[n][k] if k < num_thetas[n]
                       else (global_bound[n] if global_bound is not None
                             else theta_s[n][k - 1] + 10 * h_s[n][k - 1])
